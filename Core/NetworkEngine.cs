    ...
    public class NetworkEngine
    {
        ...
        private void LoadSettingsFromRegistry()
        {
            try
            {
                ...
                string configPath = System.IO.Path.Combine(home, "Library", "Application Support", "AgilicoConnect", "config.json");
                if (System.IO.File.Exists(configPath))
                {
                    string json = System.IO.File.ReadAllText(configPath);
                    using var doc = System.Text.Json.JsonDocument.Parse(json);
                    if (doc.RootElement.TryGetProperty("ServerUsername", out var u)) ServerUsername = u.GetString() ?? "";
                    if (doc.RootElement.TryGetProperty("ClientToken", out var t)) ClientToken = t.GetString() ?? "";
                    if (doc.RootElement.TryGetProperty("ClientUserId", out var id)) ClientUserId = id.GetInt32();
                    Log($"macOS configuration loaded. Username: {ServerUsername}, UserID: {ClientUserId}");
                }
            }
            catch (Exception ex)
            {
                Log($"Configuration loading note: {ex.Message}");
            }
        }
