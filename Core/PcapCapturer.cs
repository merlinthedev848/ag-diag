...
{
    /// <summary>
    /// Writes Wireshark-compatible PCAP data using LINKTYPE_RAW.
    /// Supports synthetic IPv4 capture and cross-platform live packet recording.
    /// </summary>
    public sealed class PcapCapturer : IDisposable
    {
        ...
        /// <summary>Creates a synthetic IPv4 packet for diagnostic logging.</summary>
        ...
        private void StartRawSocketSniffer(string? adapterIp)
        {
            ...
            catch (SocketException ex) when (ex.SocketErrorCode == SocketError.AccessDenied || ex.ErrorCode == 10013 || ex.ErrorCode == 13)
            {
                socket?.Dispose(); cts?.Dispose();
                throw new UnauthorizedAccessException("Elevated privileges (sudo/root) are required for raw packet capture on macOS/Unix.", ex);
            }
